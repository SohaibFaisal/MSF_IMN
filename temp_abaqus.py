...
def build_pbc_equations_flexible(
        instName,
        inst,
        Lx, Ly, Lz,
        xmin, xmax, ymin, ymax, zmin, zmax,
    ...
    # Use a practical geometric tolerance, not exact coordinate hashing
    faceTol = max(1e-5 * Lref, 1e-7)
    ...
    def on_max_x(x): return abs(x - xmax) <= faceTol
    def on_max_y(y): return abs(y - ymax) <= faceTol
    def on_max_z(z): return abs(z - zmax) <= faceTol

    def on_min_x(x): return abs(x - xmin) <= faceTol
    def on_min_y(y): return abs(y - ymin) <= faceTol
    def on_min_z(z): return abs(z - zmin) <= faceTol

    def find_partner(target, hitX, hitY, hitZ):
        candidates = []

        for n in all_nodes:
            x, y, z = n.coordinates

            if hitX and not on_min_x(x):
                continue
            if hitY and not on_min_y(y):
                continue
            if hitZ and not on_min_z(z):
                continue

            candidates.append(n)

        # Fallback: if strict face filter failed, use all nodes
        if not candidates:
            candidates = all_nodes

        return min(candidates, key=lambda n: d2(n.coordinates, target))
    ...
    for nP in all_nodes:
        ...
        nM = find_partner(target, hitX, hitY, hitZ)
...
def build_pbc_equations_general(
    ...
    # More realistic for Gmsh/imported meshes
    faceTol = max(1e-5 * Lref, 1e-7)
    matchTol = max(1e-5 * Lref, 1e-7)

    def instnode(label):
        return '%s.%d' % (instName, int(label))

    def is_xmax(x):
        return abs(x - xmax) <= faceTol

    def is_ymax(y):
        return abs(y - ymax) <= faceTol

    def is_zmax(z):
        return abs(z - zmax) <= faceTol
    ...
    for nP in all_nodes:
        ...
        nM = find_partner(target)

        if nM is None:
            raise RuntimeError(
                'No periodic partner found for node %d: '
                '(%.16g, %.16g, %.16g) -> (%.16g, %.16g, %.16g). '
                'faceTol=%.16g matchTol=%.16g'
                % (
                    nP.label,
                    x, y, z,
    ...
    if n_equations == 0:
        raise RuntimeError(
            'PBC generation produced ZERO equations. '
            'Detected max-face nodes: X=%d, Y=%d, Z=%d. '
            'bbox=(%.16g %.16g, %.16g %.16g, %.16g %.16g), '
            'faceTol=%.16g'
            % (nx, ny, nz, xmin, xmax, ymin, ymax, zmin, zmax, faceTol)
        )

    out.append('** Detected max-face nodes: X=%d, Y=%d, Z=%d' % (nx, ny, nz))
    out.append('** Number of generated PBC equations: %d' % n_equations)
    out.append('** ---- End PBC Equations ----')
...
if graph_main == 1:
    ...
    tol = max(1e-8 * Lref, 1e-10)
    keyTol = max(1e-8 * Lref, 1e-10)
    ...
    with open('log_' + jobName + '.txt', 'w') as f:
        f.write('FFS\n')
        mdb.jobs[jobName].writeInput(consistencyChecking=OFF)
        ...
        eq_text = build_pbc_equations_flexible(
            instName=instName,
            inst=inst,
            Lx=Lx,
            Ly=Ly,
        ...
        print('Done. Wrote robust manual RP nodes, PBC equations, and load steps:')
        f.write('Done. Wrote robust manual RP nodes, PBC equations, and load steps:')
        f.write('\n')
        print(inp_path)