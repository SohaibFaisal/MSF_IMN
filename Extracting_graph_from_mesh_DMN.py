...
def phase_onehot_from_key(phase_key: str):
    """
    Binary one-hot phase encoding used as graph node features:
      [matrix, inclusion]

    MATRIX is encoded as [1, 0]. Any non-matrix phase, e.g. UD/SFR/PR/SF,
    is encoded as [0, 1].
    """
    k = str(phase_key).upper()
    if k == "MATRIX":
...
def export_graphs_from_inp(stage: str, rve_id: str, mesh_id: str, out_dir: str = "."):
    ...
    # x_full has 8 features:
    # [vol_frac, cx, cy, cz, phase_matrix, phase_inclusion, boundary, internal]
    #
    # Notes:
    #   - vol_frac is the element volume divided by the total mesh volume.
    #   - cx/cy/cz are normalized element-centroid coordinates in [0, 1].
    #   - phase is binary one-hot: MATRIX -> [1, 0], any inclusion -> [0, 1].
    #   - boundary/internal is one-hot: boundary-touching element -> [1, 0], internal -> [0, 1].
    ...
    x_full = np.zeros((Ne, 8), dtype=np.float16)
    ...
    for ei, (eid, etype, conn) in enumerate(elements):
        ...
        x_full[ei, 2] = cy_n
        x_full[ei, 3] = cz_n
        x_full[ei, 4:6] = phase_oh
        x_full[ei, 6] = touches
        x_full[ei, 7] = 1.0 - touches
    ...
    #     x_phase = np.zeros((len(phase_eis), 8), dtype=np.float16)
