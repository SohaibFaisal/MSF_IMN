...
def export_graphs_from_inp(stage: str, rve_id: str, mesh_id: str, out_dir: str = "."):
    ...
    # Base geometry graph has 10 features:
    # [vol_n, cx_n, cy_n, cz_n, matrix, UD, SFR, PR,
    #  touches_boundary, is_target]
    #
    # The 36 material-stiffness features are appended automatically later,
    # once per material sample, by create_material_conditioned_graphs().
    ...
    x_full = np.zeros((Ne, 10), dtype=np.float32)
    vol_raw = np.zeros((Ne,), dtype=np.float32)
    ...
    FVC_full = np.asarray([V_phase[k] / V_tot for k in phase_keys], dtype=np.float32)
    ...
    np.savez_compressed(
        ...
        elem_phase_key=np.asarray(elem_phase_key),
...
if __name__ == "__main__":
    # Called by:
    # abaqus cae noGUI=Extracting_graph_from_mesh.py -- stage rve_id mesh_id
