...
def rp_for(dof, axis, rp):
    """
    Return instance-qualified RP node ids.

    RP nodes are written in a separate part instance:
        RP-1.9000001, RP-1.9000002, ...

    Never use bare 9000001 in equations or boundary conditions.
    """
    if axis == 'x':
        lbl = {1: rp['RP_E11'], 2: rp['RP_E12'], 3: rp['RP_E13']}[dof]
    elif axis == 'y':
        lbl = {1: rp['RP_E12'], 2: rp['RP_E22'], 3: rp['RP_E23']}[dof]
    elif axis == 'z':
        lbl = {1: rp['RP_E13'], 2: rp['RP_E23'], 3: rp['RP_E33']}[dof]
    else:
        raise ValueError('axis must be x, y, or z')

    return 'RP-1.%d' % int(lbl)


def make_manual_rp_block(xmax, ymax, zmax, Lref, rp):
    """
    Return two blocks:
      1) rp_part_block: inserted BEFORE *Assembly
      2) rp_asm_block: inserted INSIDE *Assembly before *End Assembly

    This fixes Abaqus errors such as:
      Unknown assembly id 9000001
    """
    x0 = xmax + 0.2 * Lref
    y0 = ymax + 0.2 * Lref
    z0 = zmax + 0.2 * Lref
    ...
    part_lines = []
    part_lines.append('** ----------------------------------------------------------------')
    part_lines.append('** Manual RP part for macro strain DOFs')
    part_lines.append('** ----------------------------------------------------------------')
    part_lines.append('*Part, name=RP_PART')
    part_lines.append('*Node')

    for i, name in enumerate(order):
        part_lines.append('%d, %.16g, %.16g, %.16g' % (
            rp[name],
            x0 + i * dx,
            y0,
            z0
    ...
    part_lines.append('** ---- End Manual RP part ----')

    asm_lines = []
    asm_lines.append('** ----------------------------------------------------------------')
    asm_lines.append('** Manual RP instance and sets for macro strain DOFs')
    asm_lines.append('** ----------------------------------------------------------------')
    asm_lines.append('*Instance, name=RP-1, part=RP_PART')
    asm_lines.append('*End Instance')

    for name in order:
        asm_lines.append('*Nset, nset=%s, instance=RP-1' % name)
        asm_lines.append('%d,' % rp[name])

    asm_lines.append('** ---- End Manual RP instance/sets ----')
...
def make_manual_bc_steps(rp, load_cases):
    ...
    # RP nodes belong to instance RP-1, so they must be referenced as RP-1.<label>.
    lines.append('** ----------------------------------------------------------------')
    ...
    for name in ['RP_E11', 'RP_E22', 'RP_E33', 'RP_E12', 'RP_E13', 'RP_E23']:
        lbl = rp[name]
        lines.append('RP-1.%d, 2, 2, 0.' % lbl)
        lines.append('RP-1.%d, 3, 3, 0.' % lbl)
    ...
    for i, lc in enumerate(load_cases):
        ...
        lines.append('*Boundary')
        lines.append('RP-1.%d, 1, 1, %.16g' % (rp['RP_E11'], E11))
        lines.append('RP-1.%d, 1, 1, %.16g' % (rp['RP_E22'], E22))
        lines.append('RP-1.%d, 1, 1, %.16g' % (rp['RP_E33'], E33))
        lines.append('RP-1.%d, 1, 1, %.16g' % (rp['RP_E12'], E12))
        lines.append('RP-1.%d, 1, 1, %.16g' % (rp['RP_E13'], E13))
        lines.append('RP-1.%d, 1, 1, %.16g' % (rp['RP_E23'], E23))
...
def strip_auto_rp_and_pbc_blocks(lines):
    markers = [
        'Manual RP part for macro strain DOFs',
        'Manual RP instance and sets for macro strain DOFs',
...
def insert_rp_and_pbc_blocks(inp_path, rp_part_block, rp_asm_block, eq_text):
    """
    Insert:
      - RP_PART before *Assembly
      - RP-1 instance, RP node sets, and PBC equations before *End Assembly
    """
    ...
    for i, ln in enumerate(lines):
        s = ln.strip().upper()
        if assembly_idx is None and s.startswith('*ASSEMBLY'):
            assembly_idx = i
        if s.startswith('*END ASSEMBLY'):
            end_assembly_idx = i
            break

    if assembly_idx is None:
        raise RuntimeError('Could not find *Assembly in %s' % inp_path)

    if end_assembly_idx is None:
        raise RuntimeError('Could not find *End Assembly in %s' % inp_path)

    part_lines = rp_part_block.splitlines(True)
    lines = lines[:assembly_idx] + part_lines + lines[assembly_idx:]

    # Account for inserted part block shifting *End Assembly index.
    end_assembly_idx += len(part_lines)

    asm_insertion = (rp_asm_block + eq_text).splitlines(True)
    lines = lines[:end_assembly_idx] + asm_insertion + lines[end_assembly_idx:]
...
if graph_main == 1:
    ...
    rp_part_block, rp_asm_block = make_manual_rp_block(
        xmax=xmax,
        ymax=ymax,
        zmax=zmax,
        Lref=Lref,
    ...
    insert_rp_and_pbc_blocks(inp_path, rp_part_block, rp_asm_block, eq_text)
    ...
    print('Done. Wrote RP_PART/RP-1 nodes, PBC equations, and steps:')
    print(inp_path)
