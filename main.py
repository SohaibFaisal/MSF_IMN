...
from Training_data_generation.data_generation import *  # importing all names in the module
...
# HYPER-PARAMETERS
# -------------------------------------
TNN_hidden_dim = 128  # question: what are these parameters?
GNN_hidden_dim = 128
X_feat = 128
GNN_structure = 4
...
if training_data_generation:
    ...
    '''
        ...
        Phase2,3,4... = 'UD1': {'FVC': fibre volume content, 'dia': [avg_dia, dia_normal_dist] (in case of SFRE1 - short fibre ellipse its 2a, norm dis of 2a, 2b, norm dist of 2b), 'ori': [avg_theta, theta_norm_dist, avg_phi, phi_norm_dist] (question: what is the norm_dist?), 'len': [avg_len, len_norm_dist]}
        For example:
        ...
        'MATRIX': {'size': [0.2, 3.0, 3.0]},
         ...
         'UED1': {'FVC': 0.25, 'dia': [0.3, 0,0.15,0], 'ori': [0, 0, np.pi / 2, 0], 'len': [3, 0]}
         }
        ...
        'MATRIX': {'size': [0.2, 3.0, 3.0]},
         ...
         'ESR1': {'FVC': 0.2, 'dia': [0.3, 0,0.15,0], 'ori': [0, 0, np.pi/2, 0], 'len': [2, 0]}}

         {'MATRIX': {'size': [3.0, 3.0, 3.0]},
         'SFR1': {'FVC': 0.2, 'dia': [0.3, 0], 'ori': [0, 0, np.pi/2, 0], 'len': [2, 0]}}
    ...
    rve_info_training_data = [
        ...
        # {'MATRIX': {'size': [3.0, 3.0, 3.0]},
        #  'SFR1': {'FVC': 0.1, 'dia': [0.3, 0,0.15,0], 'ori': [0, 0, np.pi/2, 0], 'len': [2, 0]}} # I have to make this work

        # {'MATRIX': {'size': [0.2, 3.0, 3.0]},
        #  'UD1': {'FVC': 0.25, 'dia': [0.3, 0,0.15,0], 'ori': [0, 0, np.pi / 2, 0], 'len': [3, 0]}} # even this

        # {'MATRIX': {'size': [0.2, 3.0, 3.0]},
        #  'UED1': {'FVC': 0.25, 'dia': [0.3, 0,0.15,0], 'ori': [0, 0, np.pi / 2, 0], 'len': [3, 0]}}
        # {'MATRIX': {'size': [3.0, 3.0, 3.0]},
        #  'SFR1': {'FVC': 0.25, 'dia': [0.3, 0], 'ori': [0, 0, np.pi/2, 0], 'len': [2, 0]}}
    ]
        ...
        for g_id in range(mesh_per_config):
            for attempts in range(10):
                ...
                #     pass
                    ...
                    print(f"Attempt {attempts} failed for RVE {r}, mesh {g_id}: {e}")
    ...
    # # os.system(f"abaqus python homogenize_blocks.py -- D:/MSF_IMN/{str(training_dataset_folder)} 0.01")
...
#     -------------------------------
#     '''
#     mat1 = [2, 50, 0.3, 0.2, 0.2, 0, 0]
#     mat2 = [2, 200, 0.3, 0.2, 0.2, 0, 0]
#     mat3 = [2, 200, 0.3, 0.2, 0.2, 0, 0]
#     mat4 = [2, 200, 0.3, 0.2, 0.2, 0, 0]
