...
from IMN_training.GNN_IMN import graph_runs

viz_dir = Path(r"IMN_training\graph_optuna")
...
def objective(trial):
    ...
    training_dataset_folder = Path(r"Training_data_generation\Training_data0821")  # your folder
    out_dir = Path(r"IMN_training\graph_optuna") / f"trial_{trial.number:04d}"
    ...
    try:
        ...
        x_feat = trial.suggest_categorical("x_feat", [128, 256, 512])
        ...
        # ----------------------------
        best_val = graph_runs(
            training_dataset_folder, out_dir, num_samples, optimizing_variables, num_epochs, lr, trial
        )
    ...
    if best_val is None:
        raise ValueError("best_val is None. Training did not return a valid validation loss.")

    return float(best_val), float(runtime)
...
def export_trials_csv(study, csv_path: Path):
    ...
    fieldnames = (
            ["trial_number", "state", "pruned", "val_loss", "runtime_objective",
             "best_val_loss", "runtime_sec", "epochs_ran"]
            + all_param_keys
    )
    ...
    with open(csv_path, "w", newline="") as f:
        ...
        for t in study.trials:
            ...
            row = {
                ...
                "state": state,
                "pruned": pruned,
                "val_loss": t.values[0] if t.values is not None else None,
                "runtime_objective": t.values[1] if t.values is not None else None,
...
if __name__ == "__main__":
    ...
    storage = "sqlite:///graph_opt_v2.db"  # creates a local file
    study_name = "graph_opt_v2"
    ...
    for t in study.best_trials:
        ...
        print("Validation loss:", t.values[0])
        print("Runtime sec:", t.values[1])
    ...
    study = optuna.load_study(
        study_name="graph_opt_v2",
        storage="sqlite:///graph_opt_v2.db"
    )
    ...
    fig = format_for_paper(fig)
    fig.update_layout(title=None)
    fig.write_image(viz_dir / "contour_time.pdf")
    fig.write_image(viz_dir / "contour_time.svg", scale=3)
