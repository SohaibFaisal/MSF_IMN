...
def IMN_architecture(N_layers: int, n_phases: int) -> Tuple[int, List[List[int]]]:
    Vj = []
    gaps = []
...
class IMNModel(nn.Module):
    ...
    def __init__(
        ...
        self.alpha0_list = [] # Calc
        self.Lj_list: List[int] = []
        for Vj in self.V_list:
            Lj = len(Vj)
            self.Lj_list.append(Lj)
            a0 = torch.ones(Lj, dtype=torch.double) / float(Lj)
            self.alpha0_list.append(a0)
...
class IMNCalculator:
    """
    Fast, no-grad IMN homogenization "solver".
    - Not an nn.Module
    - No trainable parameters
    - Uses flat params provided externally
    """

    def __init__(self, N_layers: int, phases: Sequence[str], init_scale: float = 1e-2,
        dtype: torch.dtype = torch.double,
        device: Optional[torch.device] = None):
        ...
        self.N, self.V_list = self.architecture(N_layers, self.n_phases)
        self.M = len(self.V_list)


        # Each node is assigned a phase cyclically
        self.node_phase = [self.phases[i % self.n_phases] for i in range(self.N)]
        ...
        # Stiffness per node (6x6 double tensor)
        self.C_nodes = torch.zeros((self.N, 6, 6), dtype=torch.double)
        # self.C_nodes = self.N * [None]
        self.H_list = torch.zeros((self.M, 6, 3), dtype=torch.double)



    def create_flat_p(self, data):
        p = [float(v) for group in zip(*(data[phase]['weights'] for phase in self.phases)) for v in group]
        p.extend([float(v) for group in zip(*(np.array(data[phase]['betas']).flatten() for phase in self.phases)) for v in group])
    ...
    # -------- stiffness assignment (from your dataset) ----------
    def assign_node_stiffness(self, data: Dict[str, np.ndarray]):
        """
        data: dict phase_key -> 6x6 stiffness (numpy or torch)
        Example keys: "MATRIX", "UD1", "UD2", ...
        """
        for i in range(self.N):
            mat = self.node_phase[i]
            ...
            if isinstance(Ci, np.ndarray):
                Ci = torch.from_numpy(Ci)
            self.C_nodes[i] = Ci.to(dtype=torch.double)

    # -------- dimensions / parameter layout ----------
    def param_dim(self) -> int:
        """
        Flat params layout used by homogenize_from_flat_params:
          z(0:N) + for each join j: theta_j, phi_j, beta_j(0:Lj-1)
        """
        P = self.N
    ...
    def _H_from_theta_phi(theta_raw: torch.Tensor, phi_raw: torch.Tensor) -> torch.Tensor:
        """
        theta_raw, phi_raw are unconstrained -> mapped to angles.
        """
        theta = torch.pi * torch.sigmoid(theta_raw)
        phi = 2.0 * torch.pi * torch.sigmoid(phi_raw)
        n = torch.stack([
    ...
    def build_H_list(self, theta_raw: torch.Tensor, phi_raw: torch.Tensor) -> torch.Tensor:
        """
        theta_raw, phi_raw: (M,) each
        returns list of M tensors 6x3
        """
        # H_list = []
        for j in range(self.M):
            self.H_list[j, :, :] = self._H_from_theta_phi(theta_raw[j], phi_raw[j])
        return self.H_list

    def compute_alpha_matrix(self, W: torch.Tensor, betas, device) -> torch.Tensor:
        alpha = torch.zeros((self.N, self.M), dtype=torch.double, device=device)

        W = W.to(dtype=torch.double)  # <-- add this once

        for j, Vj in enumerate(self.V_list):
            idx = torch.tensor(Vj, dtype=torch.long, device=device)

            Wc = W[idx]  # now double
            beta_j = betas[idx, j].to(dtype=torch.double, device=device)

            a0 = beta_j[0]  # double

            alpha_children = compute_alphas_for_join(Wc, beta_j[1:], a0)  # will be double
            alpha[idx, j] = alpha_children

        return alpha

    def build_D_list(self, alpha: torch.Tensor, H_list: torch.Tensor, device) -> torch.Tensor:
        """
        returns list length N, each Di is (6, 3M)
        """
        # D_list = []
        D_list = torch.zeros((self.N, 6, 3*self.M), dtype=torch.double, device=device)
        for i in range(self.N):
            Di = torch.zeros((6, 3 * self.M), dtype=torch.double, device=device)
            for j in range(self.M):
                Di[:, 3*j:3*(j+1)] = alpha[i, j] * H_list[j,:,:]
    ...
    # -------- homogenization ----------
    def homogenize(self, W: torch.Tensor, theta_raw: torch.Tensor, phi_raw: torch.Tensor,
        ...
        device = W.device
        H_list = self.build_H_list(theta_raw, phi_raw)
        alpha = self.compute_alpha_matrix(W, betas, device)
        D_list = self.build_D_list(alpha, H_list, device)
    ...
    # -------- flat param interface ----------

    def homogenize_from_flat_params(self, p_hat_1d: torch.Tensor) -> torch.Tensor:
        """
        p_hat_1d layout:
          z(0:N) + [theta_j, phi_j, beta_j(0:Lj-1)] for j in 0..M-1
        """
        if p_hat_1d.ndim != 1:
            p_hat_1d = p_hat_1d.view(-1)

        device = p_hat_1d.device
        idx = 0

        z = p_hat_1d[idx:idx+self.N];     idx += self.N
        W = z.to(dtype=torch.double)



        betas_mat = p_hat_1d[idx: idx + self.M * self.N].view(self.N, self.M)

        idx += self.M * self.N


        theta_phi = p_hat_1d[idx: idx + 2 * self.M].view(self.M, 2)
        theta_raw = theta_phi[:, 0].to(dtype=torch.double)
        phi_raw = theta_phi[:, 1].to(dtype=torch.double)
    ...
    # -------- diagnostics ----------
    @staticmethod
    def normalized_frobenius_mse(C_pred: torch.Tensor, C_tgt: torch.Tensor, eps: float = 1e-12) -> torch.Tensor:
        diff = C_pred - C_tgt
        num = (diff * diff).sum()
        den = (C_tgt * C_tgt).sum() + eps
        return num / den
    ...
    def phase_fraction_error(self, p_hat_1d: torch.Tensor, W_phases: Sequence[float]) -> torch.Tensor:
        """
        Diagnostic: compares phase weight sums against target phase fractions.
        Assumes node phases repeat cyclically (same as node_phase assignment).
        """
        if p_hat_1d.ndim != 1:
            p_hat_1d = p_hat_1d.view(-1)
        device = p_hat_1d.device
        idx = 0
        z = p_hat_1d[idx:idx + self.N]
        idx += self.N
        W = z.to(dtype=torch.double)
        W_target = torch.as_tensor(W_phases, dtype=W.dtype, device=W.device)
        p = self.n_phases
        ...
        diff = phase_sums - W_target
        num = (diff * diff).sum()
        den = (W_target * W_target).sum()

        return num / den


    def output_params_from_p_flat(self, p_hat_1d: torch.Tensor, FEAP_path):
        ...
        H_list = self.build_H_list(theta_raw, phi_raw)
        alpha = self.compute_alpha_matrix(W, betas_mat, device)
        D_list = self.build_D_list(alpha, H_list, device)

        D = np.asfortranarray(D_list, dtype=np.float64)
        D.ravel(order="F").tofile(FEAP_path / "D_0.bin")
        shape_D = np.array(D.shape, dtype=np.int32)
        shape_D.tofile(FEAP_path / f"D_0.shape")
        ...
        W.ravel(order="F").tofile(FEAP_path / f"w_0.bin")
        shape_W = np.array(W.shape, dtype=np.int32)
        shape_W.tofile(FEAP_path / "w_0.shape")
