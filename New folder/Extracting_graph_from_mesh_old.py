...
def phase_id_from_key(phase_key: str) -> float:
    """
    Scalar phase encoding used by the old 7-feature graph format:
      MATRIX -> 1
      UD     -> 2
      PR     -> 3
      SF/SFR -> 4
...
def export_graphs_from_inp(stage: str, rve_id: str, mesh_id: str, out_dir: str = "."):
    ...
    # Build adjacency: graph nodes are finite elements; edges connect elements
    # sharing at least one mesh node.
    node2elems = build_node2elems(elements)
    edge_index_full = build_edge_index(node2elems)

    # Old 7-feature node format:
    # [vol_frac, cx, cy, cz, phase_id, touches_boundary, internal]
    # phase_id: MATRIX=1, UD=2, PR=3, SF/SFR=4
    # touches_boundary/internal are one-hot boundary indicators.
    ...
    x_full = np.zeros((Ne, 7), dtype=np.float16)
    vol_raw = np.zeros((Ne,), dtype=np.float16)

    # First pass: raw volumes/centroids + scalar phase ids + boundary flags
    for ei, (eid, etype, conn) in enumerate(elements):
        ...
        x_full[ei, 0] = vol  # normalized after total volume is known
        x_full[ei, 1] = cx_n
        x_full[ei, 2] = cy_n
        x_full[ei, 3] = cz_n
        x_full[ei, 4] = pid
        x_full[ei, 5] = touches
        x_full[ei, 6] = internal

    # Phase volumes from exclusive assignment
    ...
    # Normalize element volume by total RVE volume -> each element stores volume fraction.
    x_full[:, 0] = x_full[:, 0] / V_tot

    os.makedirs(out_dir, exist_ok=True)

    # Save full graph
    out_full = os.path.join(out_dir, f"graph_stage_{stage}_rve_{rve_id}_mesh_{mesh_id}_old.npz")
    np.savez_compressed(
        out_full,
        x=x_full,
    ...
    # Save per-phase subgraphs using the same 7-feature format.
    ...
    for pkey in phase_keys:
        ...
        safe_pkey = re.sub(r"[^A-Za-z0-9_.-]+", "_", str(pkey))
        out_phase = os.path.join(
            out_dir,
            f"graph_stage_{stage}_rve_{rve_id}_mesh_{mesh_id}_phase_{safe_pkey}_old.npz",
        )
        np.savez_compressed(
            out_phase,
            x=x_phase,
